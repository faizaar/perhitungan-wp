    ...
    <div class="col-12">
      <div class="card mb-4">
        <div class="card-header pb-0">
          <h6>Data Konversi</h6>
          <button class="btn btn-primary btn-sm" onclick="window.location.href='/Home/tambahkonversi'">Tambah Data</button>
        </div>
                ...
                <?php if (!empty($dataMb)): ?>
                  <?php foreach ($dataMb as $konversi): ?>
                    <tr>
                      ...
                      <td>
                        <p class="text-xs font-weight-bold mb-0"><?php echo $konversi->polisi_id; ?></p>
                      </td>
                      <td>
                        <p class="text-xs font-weight-bold mb-0"><?php echo $konversi->kriteria_id; ?></p>
                      </td>
                      <td>
                        <p class="text-xs font-weight-bold mb-0"><?php echo $konversi->nilai; ?></p>
                      </td>
                      <td class="text-center">
                        <a href="<?= base_url('/Home/editkonversi/' . $konversi->id); ?>" class="btn btn-warning btn-sm">Update</a>
                        <a href="<?= base_url('/Home/deletekonversi/' . $konversi->id); ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus data ini?');">Delete</a>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                <?php else: ?>
